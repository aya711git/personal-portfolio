# 🌸 Personal Portfolio

A modern and responsive personal portfolio website built with **React**, designed to showcase my work, skills, and experience as a **Frontend Developer**.


## 🚀 Features
- 🖼️ Elegant and responsive design  
- 🌙 Dark/Light mode toggle  
- 💼 Projects section with details and previews  
- 📜 About & Skills sections  
- 📞 Contact form integration  
- 🌐 Multi-language support (Arabic / English)



## 🛠️ Tech Stack
- **React** (Vite or Next.js if applicable)
- **TypeScript** *(if used)*
- **Tailwind CSS** / **ShadCN UI** for styling  
- **Framer Motion** for animations  
- **Lucide React** for icons  
- **EmailJS** or any API for contact form  



## 🧩 Project Structure
```

src/
├─ components/
├─ pages/
├─ assets/
├─ styles/
└─ utils/

````



## ⚙️ Setup & Installation
1. Clone the repository:
   ```bash
   git clone https://github.com/aya711git/personal-portfolio.git
   ```

2. Navigate to the project folder:

   ```bash
   cd personal-portfolio
   ```
3. Install dependencies:

   ```bash
   npm install
   ```
4. Start the development server:

   ```bash
   npm run dev
   ```

## 🧠 Future Improvements

* Add a blog section ✍️
* Integrate custom animations 💫
* Add backend support for messages 📨


## بالعربية

هذا المشروع هو موقعي الشخصي الذي يعرض أعمالي ومهاراتي كمطوّرة واجهات أمامية.  
صُمّم بواجهة حديثة تدعم الوضعين الداكن والفاتح، ويهدف إلى تقديم نموذج مميز للمبرمجين العرب المهتمين بتصميم واجهات تفاعلية احترافية.  

🌷 أطمح أن أساهم في إثراء المحتوى العربي في مجال تطوير الواجهات الأمامية،  
وألهم غيري من المطورين والمطورات العرب للانطلاق بثقة في بناء مشاريعهم الخاصة.


## 📩 Contact

**Aya Khaled Khrais**
🌐 [GitHub](https://github.com/aya711git)
📧[Email](aya.khuris@gmail.com)
🖇️[LinkedIn](https://www.linkedin.com/in/aya-khaled-khuris/)
💼 Frontend Developer | Passionate about modern UI/UX |AI Research

---
✨ شكرًا لزيارتك، وأتمنى أن تجد في هذا المشروع فائدة وإلهامًا 💛  
✨ Thank you for visiting! I hope you find inspiration and value in this project 💛


